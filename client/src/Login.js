...
class Login extends Component {
  ...
  handleSubmit = e => {
    ...
    axios.get("http://localhost:3001/api/authenticate", { params: login }).then(res => {
      ...
      if (res.data.status === true) {
        ...
        sessionStorage.setItem('suid', res.data.User_Id);
      } else {
        this.setState({ fireRedirectError: true });
  ...
  renderRedirect = () => { return <Redirect to={`/booking/${this.state.User_Id}`} /> }
  renderRedirectError = () => { return <Redirect to={`/error`} /> }
  ...
  render() {
    return <div>
      ...
      {(this.state.fireRedirectError) ? this.renderRedirectError() : null}
    </div>
  }
}
